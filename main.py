...
class FuncDec(Node):
    ...
    def evaluate(self,table):
        if table.order:
            name = table.order.pop(0)
            if  name != self.value:
                raise Exception("Error - in FuncDec, name of function not in accordance with NATO phonetic alphabet, expected:",name," received : ",self.value)
...
class SymbolTable():
    def __init__(self):
        ...
        self.order = ["alfa", "bravo", "charlie", "delta", "echo", "foxtrot", "golf", "hotel", "india", "juliett", "kilo", 'lima', 'mike', 'november', 'oscar', 'papa', 'quebec', 'romeo', 'sierra', 'tango', 'uniform', 'victor', 'whiskey', 'x-ray', 'yankee', 'zulu']
...
class parser:
    ...
    def RelExpression():
        ...
        if parser.token.actual.stamp == "BIGGER":
            parser.token.selectNext()
            return BinOp(">",[result,parser.parseExpression()])
        if parser.token.actual.stamp == "SMALER":
            parser.token.selectNext()
            return BinOp("<",[result,parser.parseExpression()])
        return result
            ...
            if parser.token.actual.stamp == "SUB":
                ...
                if  parser.token.actual.stamp == "SUB":
                    ...
                    if  parser.token.actual.stamp == "FIN":
                        ...
                        raise Exception("Error - wrong order at ending expected:","over","received:", parser.token.actual.value)
                        
                else:
                    raise Exception("Error - wrong order at ending expected:","procedure","received:", parser.token.actual.value)
